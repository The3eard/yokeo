...
var datetimepickerFactory = function ($) {
	...
	$.datetimepicker = {
		setLocale: function (locale) {
			var newLocale = default_options.i18n[locale]
				? locale
				: globalLocaleDefault;
		...
		setDateFormatter: function (dateFormatter) {
			if (
				...
				dateFormatters.hasOwnProperty(dateFormatter)
			) {
	...
	$.fn.xdsoftScroller = function (options, percent) {
		return this.each(function () {
			...
			if (!$(this).hasClass('xdsoft_scroller_box')) {
				...
				scroller
					.on('touchstart.xdsoft_scroller mousedown.xdsoft_scroller', function (
						event
					) {
						if (!parentHeight) {
							...
							$([options.ownerDocument.body, options.contentWindow]).on(
								...
								function arguments_callee() {
									$([options.ownerDocument.body, options.contentWindow])
										.off('touchend mouseup.xdsoft_scroller', arguments_callee)
										.off('mousemove.xdsoft_scroller', calcOffset)
										.removeClass('xdsoft_noselect');
								}
							);
							$(options.ownerDocument.body).on(
					...
					.on('scroll_element.xdsoft_scroller', function (event, percentage) {
						...
						setTimeout(function () {
							timebox.css(
								'marginTop',
								-parseInt(
									(timebox[0].offsetHeight - parentHeight) * percentage,
									10
								)
							);
						}, 10);
					...
					.on('resize_scroll.xdsoft_scroller', function (
						...
						if (percent > 1) {
							...
							if (noTriggerScroll !== true) {
								timeboxparent.trigger('scroll_element.xdsoft_scroller', [
									percentage ||
										Math.abs(parseInt(timebox.css('marginTop'), 10)) /
											(height - parentHeight),
								]);
							}
						}
				...
				timeboxparent.on('touchmove', function (event) {
					if (start) {
						...
						timeboxparent.trigger('scroll_element.xdsoft_scroller', [
							(startTop - (coord.y - start.y)) / (height - parentHeight),
						]);
					}
				});
	...
	$.fn.datetimepicker = function (opt, opt2) {
		...
		createDateTimePicker = function (input) {
			...
			month_picker
				...
				.on('touchstart mousedown.xdsoft', function (event) {
					...
					if (_xdsoft_datetime.currentTime) {
						val = _xdsoft_datetime.currentTime[
			...
			var handleTouchMoved = function (event) {
				...
				var xMovement = Math.abs(
					this.touchStartPosition.clientX - touchPosition.clientX
				);
				var yMovement = Math.abs(
					this.touchStartPosition.clientY - touchPosition.clientY
				);
			...
			datetimepicker.setOptions = function (_options) {
				...
				if (
					_options.allowDates &&
					$.isArray(_options.allowDates) &&
					_options.allowDates.length
				) {
					options.allowDates = $.extend(true, [], _options.allowDates);
				}

				if (
					_options.allowDateRe &&
					Object.prototype.toString.call(_options.allowDateRe) ===
						'[object String]'
				) {
					...
					$.each(_options.highlightedDates, function (index, value) {
						var splitData = $.map(value.split(','), $.trim),
							...
							hDate = new HighlightedDate(
								dateHelper.parseDate(splitData[0], options.formatDate),
								splitData[1],
								splitData[2]
							), // date, desc, style
				...
				if (options.minDate && /^[\+\-](.*)$/.test(options.minDate)) {
					options.minDate = dateHelper.formatDate(
						_xdsoft_datetime.strToDateTime(options.minDate),
						options.formatDate
					);
				}

				if (options.maxDate && /^[\+\-](.*)$/.test(options.maxDate)) {
					options.maxDate = dateHelper.formatDate(
						_xdsoft_datetime.strToDateTime(options.maxDate),
						options.formatDate
					);
				}

				if (options.minDateTime && /^\+(.*)$/.test(options.minDateTime)) {
					options.minDateTime = _xdsoft_datetime
						.strToDateTime(options.minDateTime)
						.dateFormat(options.formatDate);
				}

				if (options.maxDateTime && /^\+(.*)$/.test(options.maxDateTime)) {
					options.maxDateTime = _xdsoft_datetime
						.strToDateTime(options.maxDateTime)
						.dateFormat(options.formatDate);
				}
			...
			XDSoft_datetime = function () {
				...
				_this.getWeekOfYear = function (datetime) {
					if (
						options.onGetWeekOfYear &&
						$.isFunction(options.onGetWeekOfYear)
					) {
					...
					return Math.ceil(
						((datetime - onejan) / 86400000 + onejan.getDay() + 1) / 7
					);
				};
				...
				_this.strToDateTime = function (sDateTime) {
					...
					if (tmpDate && tmpDate[2]) {
						timeOffset =
							tmpDate[2].getTime() - tmpDate[2].getTimezoneOffset() * 60000;
						...
						currentTime = sDateTime
							? dateHelper.parseDate(sDateTime, options.format)
							: _this.now();
					}
				...
				_this.strToDate = function (sDate) {
					...
					var currentTime = sDate
						? dateHelper.parseDate(sDate, options.formatDate)
						: _this.now(true);
					if (!_this.isValidDate(currentTime)) {
				...
				_this.strtotime = function (sTime) {
					...
					var currentTime = sTime
						? dateHelper.parseDate(sTime, options.formatTime)
						: _this.now(true);
					if (!_this.isValidDate(currentTime)) {
						currentTime = _this.now(true);
					}
				...
				_this.str = function () {
					...
					if (options.yearOffset) {
						format = format.replace(
							'Y',
							_this.currentTime.getFullYear() + options.yearOffset
						);
						format = format.replace(
							'y',
							String(
				...
				.on('touchend mousedown.xdsoft', function () {
					...
					$([options.ownerDocument.body, options.contentWindow]).on(
						...
						function arguments_callee2() {
							clearTimeout(timer);
							stop = true;
							$([options.ownerDocument.body, options.contentWindow]).off(
								'touchend mouseup.xdsoft',
								arguments_callee2
							);
						}
			...
			timepicker
				...
				.on('touchend mousedown.xdsoft', function () {
					...
					(function arguments_callee4(v) {
						...
						timeboxparent.trigger('scroll_element.xdsoft_scroller', [
							Math.abs(
								parseInt(timebox[0].style.marginTop, 10) / (height - pheight)
							),
						]);
				...
				.on('xchange.xdsoft', function (event) {
					...
					xchangeTimer = setTimeout(function () {
						...
						if (maxDateTime !== false) {
							maxDateTimeDay =
								(maxDateTime.getFullYear() * 12 + maxDateTime.getMonth()) * 31 +
								maxDateTime.getDate();
						}

						while (
							i < _xdsoft_datetime.currentTime.countDaysInMonth() ||
							...
							if (
								options.allowDateRe &&
								Object.prototype.toString.call(options.allowDateRe) ===
									'[object RegExp]'
							) {
								if (
									!options.allowDateRe.test(
										dateHelper.formatDate(start, options.formatDate)
									)
								) {
							...
							var currentDay =
								(start.getFullYear() * 12 + start.getMonth()) * 31 +
								start.getDate();
							if (
							...
							if (
								dateHelper.formatDate(today, options.formatDate) ===
								dateHelper.formatDate(start, options.formatDate)
							) {
						...
						if (options.minDateTime !== false) {
							...
							var currentDayIsMinDateTimeDay =
								dateHelper.formatDate(
									_xdsoft_datetime.currentTime,
									options.formatDate
								) === dateHelper.formatDate(t, options.formatDate);
						...
						if (options.maxDateTime !== false) {
							...
							var currentDayIsMaxDateTimeDay =
								dateHelper.formatDate(
									_xdsoft_datetime.currentTime,
									options.formatDate
								) === dateHelper.formatDate(t, options.formatDate);
						...
						line_time = function line_time(h, m) {
							...
							current_time.setHours(
								parseInt(_xdsoft_datetime.currentTime.getHours(), 10)
							);
							...
							if (
								...
								((!isALlowTimesInit && options.step > 59) ||
									current_time.getMinutes() === parseInt(m, 10))
							) {
							...
							if (
								parseInt(today.getHours(), 10) === parseInt(h, 10) &&
								parseInt(today.getMinutes(), 10) === parseInt(m, 10)
							) {
								classes.push('xdsoft_today');
							}
							time +=
			...
			input.on('mousewheel.xdsoft', function (event) {
				...
				if (!options.datepicker && options.timepicker) {
					...
					if (
						...
						current_time_index + event.deltaY < timebox.children().length
					) {
			...
			setPos = function () {
				...
				if (
					options.ownerDocument.documentElement.clientWidth -
						dateInputOffset.left <
					datepicker.parent().outerWidth(true)
				) {
					var diff =
						datepicker.parent().outerWidth(true) - dateInputElem.offsetWidth;
					left = left - diff;
				}
				...
				if (options.fixed) {
					...
					if (dateInputHasFixedAncestor && !options.insideParent) {
						...
						if (
							verticalPosition + datetimepicker.outerHeight() >
							windowHeight + windowScrollTop
						) {
							verticalAnchorEdge = 'bottom';
							verticalPosition =
								windowHeight + windowScrollTop - dateInputOffset.top;
						} else {
							verticalPosition -= windowScrollTop;
						}
					} else {
						if (
							verticalPosition + datetimepicker[0].offsetHeight >
							windowHeight + windowScrollTop
						) {
							verticalPosition =
								dateInputOffset.top - datetimepicker[0].offsetHeight + 1;
						}
					}
				...
				forEachAncestorOf(datetimepickerElem, function (ancestorNode) {
					...
					ancestorNodePosition = options.contentWindow
						.getComputedStyle(ancestorNode)
						.getPropertyValue('position');

					if (
						ancestorNodePosition === 'relative' &&
						windowWidth >= ancestorNode.offsetWidth
					) {
				...
				if (options.insideParent) {
					datetimepickerCss[verticalAnchorEdge] =
						dateInputElem.offsetTop + dateInputElem.offsetHeight;
				} else {
			...
			function getCurrentValue() {
				...
				if (options.startDate) {
					...
					if (ct) {
						...
						if (options.yearOffset) {
							ct = new Date(
								ct.getFullYear() - options.yearOffset,
								ct.getMonth(),
								ct.getDate(),
								ct.getHours(),
								ct.getMinutes(),
			...
			function setMask(options) {
				var isValidValue = function (mask, value) {
					...
					getCaretPos = function (input) {
						try {
							if (
								options.ownerDocument.selection &&
								options.ownerDocument.selection.createRange
							) {
					...
					setCaretPos = function (node, pos) {
						node =
							typeof node === 'string' || node instanceof String
								? options.ownerDocument.getElementById(node)
								: node;
						if (!node) {
							return false;
						}
					...
					input.on('keydown.xdsoft', function (event) {
						...
						if (
							...
							while (true) {
								...
								var curPosOnSep = notNumberOrPlaceholder.test(
									maskValueAtCurPos
								);
								var continueMovingPosition =
									curPosOnSep && posShorterThanMaskLength && posGreaterThanZero;
							...
							if (hasSel) {
								...
								var defaultBlankSelectionReplacement = defaultBlank.substr(
									pos,
									selLength
								);
								var selReplacementRemainder = defaultBlankSelectionReplacement.substr(
									1
								); // might be empty
							...
							if (
								([AKEY, CKEY, VKEY, ZKEY, YKEY].indexOf(key) !== -1 &&
									ctrlDown) ||
								[
									ESC,
									ARROWUP,
			...
			input
				...
				.on(
					...
					function () {
						if (
							input.is(':disabled') ||
							(input.data('xdsoft_datetimepicker').is(':visible') &&
		...
		destroyDateTimePicker = function (input) {
			...
			if (datetimepicker) {
				...
				$([options.contentWindow, options.ownerDocument.body]).off(
					...
					switch (opt) {
						...
						case 'reset':
							...
							if (
								...
								!datetimepicker
									.data('xdsoft_datetime')
									.isValidDate(dateHelper.parseDate(this.value, options.format))
							) {
